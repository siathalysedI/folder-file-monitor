...
echo "  - Full file paths in logs and status"
echo "  - Date/time stamped error logging"
echo "  - Status shows last 7 days by default"
echo "  - Recent command accepts hours parameter"
echo ""
echo "Main commands:"
echo "   $SCRIPT_FILE status          - View status (last 7 days)"
echo "   $SCRIPT_FILE recent          - View last 24 hours"
echo "   $SCRIPT_FILE recent 6        - View last 6 hours"
echo "   $SCRIPT_FILE recent 168      - View last 7 days"
...
echo ""
echo "Your system now automatically monitors all changes with enhanced tracking."
    fi
...
# 3. Create main script with enhanced functionality
...
curl -fsSL https://raw.githubusercontent.com/siathalysedI/folder-file-monitor/main/folder_file_monitor.sh -o "$SCRIPT_FILE" 2>/dev/null || {
    echo "   Could not download from GitHub, creating local script..."
